...
const ProfileDropdown = ({ 
    ...
    onToggle, 
    avatar, 
    user = {},  // ✅ Change to accept user object
    onLogout 
}) => {
    const navigate = useNavigate();
     // ✅ FIX: Use proper extraction with fallback
    const companyName = user.name || user.username || "User";
    const email = user.email || "";  // Make sure this line exists

    // ✅ Add detailed debug
    console.log('ProfileDropdown - Full user object:', JSON.stringify(user));
    console.log('ProfileDropdown - user.email:', user.email);
    console.log('ProfileDropdown - extracted email:', email);
    return (
